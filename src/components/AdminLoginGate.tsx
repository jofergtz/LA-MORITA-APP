...
import { ShieldAlert, Key, HelpCircle, ArrowLeft, Eye, EyeOff, User as UserIcon, X } from 'lucide-react';
...
export default function AdminLoginGate({ onVerify, onCancel, isModal = false }: AdminLoginGateProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!username.trim() || !passcode.trim()) {
      setError('Por favor, ingresá el usuario y la contraseña de administración.');
      return;
    }
  ...
  const content = (
    <div className="bg-white border border-morita-sand rounded-3xl p-6 sm:p-8 shadow-xl max-w-md w-full relative animate-fade-in">
      ...
      <div className="flex flex-col items-center text-center mb-6">
        <div className="bg-morita-terracotta/10 text-morita-terracotta p-3.5 rounded-full mb-3 shadow-xs">
          <ShieldAlert className="h-9 w-9 animate-pulse" />
        </div>
        <h2 className="text-xl font-serif font-bold text-morita-charcoal tracking-tight">
          Ingreso a Modo Administración
        </h2>
        <p className="text-xs text-morita-charcoal/60 mt-1.5 leading-relaxed">
          Ingresá tus credenciales de moderador para administrar la comunidad de **La Morita**.
        </p>
      </div>
      ...
      {error && (
        <div className="mb-5 bg-red-50 border border-red-200 text-red-800 text-xs font-semibold p-3.5 rounded-xl leading-relaxed">
          {error}
          {attempts >= 2 && (
            <p className="mt-1.5 text-[10px] text-red-600/80 font-normal">
              Recuerda: Usuario: <strong className="font-bold">admin</strong> | Contraseña: <strong className="font-bold">admin2026</strong>
            </p>
          )}
        </div>
      )}
      ...
      <div className="relative my-5 text-center">
        <div className="absolute inset-0 flex items-center">
          <div className="w-full border-t border-morita-sand/50"></div>
        </div>
        <span className="relative bg-white px-3 text-[10px] font-bold uppercase tracking-wider text-morita-charcoal/40">
          Credenciales del Sistema
        </span>
      </div>

      {/* Demo Fast Access Help Card */}
      <div className="bg-purple-50/70 border border-purple-100 rounded-2xl p-3.5 space-y-2">
        <div className="flex items-start space-x-2">
          <HelpCircle className="h-4 w-4 text-purple-700 shrink-0 mt-0.5" />
          <div>
            <span className="block text-[10px] font-bold uppercase text-purple-900">Credenciales de Acceso Admin:</span>
            <p className="text-[11px] text-purple-950 leading-snug mt-0.5">
              Usuario: <strong className="font-bold text-purple-900">admin</strong>
              <br />
              Contraseña: <strong className="font-bold text-purple-900">admin2026</strong>
            </p>
          </div>
        </div>
        <button
          onClick={handleAutoFill}
          className="w-full bg-white hover:bg-purple-100/50 border border-purple-200 text-purple-900 font-bold text-[10px] py-1.5 rounded-lg transition-all cursor-pointer shadow-2xs"
        >
          ⚡ Completar credenciales automáticas
        </button>
      </div>
